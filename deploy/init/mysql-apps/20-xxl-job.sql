-- 来源：XXL-JOB 3.4.2 官方 tables_xxl_job.sql（Copyright xuxueli）。
-- 仅保留表结构、调度锁与官方默认 admin 引导账号；去掉示例执行器与 AI 样例任务。
-- 默认 admin 密码哈希对应官方文档口令 123456，仅用于空库首次登录，本地必须立即修改，不是本仓库密钥。
-- 本文件位于 deploy/，不作为业务 DDL 门禁扫描对象。库由 00-users.sh 创建。
