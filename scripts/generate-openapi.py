    ...
    FulfillmentCreateRequest:
      ...
      properties:
        ...
        ownerId:
          ...
          minLength: 1
          maxLength: 64
          description: "运行消息启用时必填；旧未知货主不得自动授权。"
    ...
    SourceWindowPage:
      ...
      properties:
        ...
        facts:
          ...
          items:
            type: object
            required: [commandId, action, executionId, quantity, postedQuantity, postingId, occurredAt, resultState]
            properties:
              commandId: { $ref: '#/components/schemas/Id' }
              ...
              postingId: { type: [string, 'null'], maxLength: 64 }
              occurredAt: { type: string, format: date-time }
              resultState: { type: string, enum: [APPLIED, REJECTED, CANCELLED], description: 拒绝或取消必须是零过账且无postingId，接收方仍须核对库存终态 }
