-- S7-02 入库库任务身份与分片。只写本库。XXL 触发不等于分片完成。
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='入库任务运行，调度成功不等于本行完成';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='入库分片租约与检查点，回收后旧 fence 失效';
