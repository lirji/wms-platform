...
public interface SourceMapper {
    @Insert("INSERT INTO source_effect (id, enterprise_id, warehouse_id, source_service, action, fact_type, fact_parent_id, "
            ...
            + "#{factType}, #{factParentId}, #{factPartId}, #{factLineId}, #{id}, #{commandId}, NULL, 1, #{state}, 0, #{now}, "
    ...
    @Insert("INSERT IGNORE INTO source_command (id, enterprise_id, warehouse_id, command_id, source_operation_id, "
            ...
            + "digest_version, payload_json, state, inventory_operation_id, posting_id, retry_at, compensates_command_id, "
            + "version, created_at, updated_at) VALUES (#{commandId}, #{enterpriseId}, #{warehouseId}, #{commandId}, "
            + "#{operationId}, #{executionId}, #{effectId}, #{action}, #{commandId}, 1, #{digest}, 1, CAST(#{payload} AS JSON), "
            + "#{state}, NULL, NULL, NULL, NULL, 0, #{now}, #{now})")
