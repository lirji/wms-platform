...
public final class SourceWindowController {
    ...
    public org.springframework.http.ResponseEntity<Map<String,Object>> collect(@AuthenticationPrincipal Jwt jwt,
        ...
        try(var session=sessions.openSession(false)) {
            var result=new SourceWindowService(session,Clock.systemUTC(),"wms-outbound").collect(e,warehouseId,cutoffId,Instant.parse(body.cutoff()));
            Map<String,Object> response=Map.of("schemaVersion",1,"sourceService","wms-outbound","enterpriseId",e,
    ...
    public SourceWindowService.Page facts(@AuthenticationPrincipal Jwt jwt,@PathVariable String warehouseId,
        ...
        String e=require(jwt,warehouseId);
        try(var session=sessions.openSession(false)) {
            return new SourceWindowService(session,Clock.systemUTC(),"wms-outbound").read(e,warehouseId,cutoffId,Instant.parse(cutoff),cursor);
        }
    }
