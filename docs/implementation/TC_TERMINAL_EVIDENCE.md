...
S0-09隔离POC，尚未批准为生产方案。沿用跨仓Seata TCC及“固定分支全部CONFIRMED + TC可靠全局成功证据”门禁；本实验没有业务RM，不替代S0-05a或AC-12/42验收。
...
DB探针断言事务begin后真实出现在`global_table`，以排除配置没有生效。随后检查提交/回滚终态证据与会话清理、TC重启后查询恢复、缺失XID无记录，以及审计写失败后的TC重试恢复。测试SQL只存在于`src/test/resources/db/tc-probe`，不会被三个业务服务自动执行。
...
- 带两个真实库存RM、持久化Fence及分支路由的提交/回滚；TC/RM/TM各崩溃窗口、超时回滚、重复/乱序二阶段回调。
