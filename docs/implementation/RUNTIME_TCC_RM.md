...
原意图进入58张仓范围复制清单，Fence另按原XID/branch及可靠TC终态精确迁移。缺终态、本地仍在途或原来源缺失时继续拒绝切流；目标保留原应用和资源身份接收只读历史回调。实际TC/Kafka/迁移/重启和TC回调重放证据见[TC资源迁移](TC_RESOURCE_MIGRATION.md)。下面04:00阶段的测试数量与限制作为历史记录，不替代当前门禁。
