...
| 4 | R13–R15 | 消息闭环、TC/序列号服务、恢复与清理任务 | in progress；RECEIVE/分批QUALITY/PUTAWAY双进程通过；登记HTTP/有界调用/持久化恢复与受审计重排已验证；R15七个handler已有执行器（归档仅候选）；TC只读审计/有界恢复和分支屏障、库存确认到履约可靠消费及Inbox兼容迁移已定向验证；PICK/SHIP/CANCEL真实双进程、原订单行分批/并发、桶级发运额度和取消回执已验证；完整授权投递、同事务消费与取消签发边界已定向验证；仍待序列号观察、盘点逐身份登记、可信水位与真实TM/RM |
