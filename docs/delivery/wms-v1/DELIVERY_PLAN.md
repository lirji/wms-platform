...
| 4 | R13–R15 | 消息闭环、TC/序列号服务、恢复与清理任务 | in progress；普通RECEIVE/分批QUALITY/PUTAWAY及PICK/SHIP/CANCEL真实双进程通过；七个handler已有有界执行器（归档仅候选）；完整授权、原生RM、持久TM执行及原XID/branch到出库授权真实进程验证通过；多cell普通库存路由已发布；按收货批次的序列号观察与身份质检已定向验证并提交；序列号上架已通过真实进程与事务故障验证；源释放持久事实/历史凭证重放/隔离恢复已验证；盘点完整身份输入/空集合/重放与HTTP已验证。仍待序列出库、公开序列调拨接线、TC终态通知/资源迁移、取消补偿、盘点逐身份登记与可信水位 |
| 4 | R21/R24 | 就绪/观测、正式装配测试、容量执行器 | 核心已提交并本地验证；消息积压/采样有效性指标和告警检查已定向通过，生产通知未接；容量目标未签署，执行器测试不等于WMS容量达标 |
| 5 | R22–R23 | UTC 序列化、操作者审计 | R23已验证；R22数据库时间来源/UTC映射/历史固定偏移/跨JVM分页已验证，等待最终组合与远程CI；没有转换或声明生产历史时区 |
...
| --- | --- | --- | --- | --- | --- |
| OUT：具体SN分次发运并恢复 | 原订单行/桶/epoch；T1/T2/T3；独立SHIPPED证明；丢回执不重复扣量；旧执行器不覆盖；可查询/审计重排 | PICK dd22cd0；outbound V018、inventory V042、registry V005；有界HTTP与既有作业 | MySQL IT、公开HTTP、实际JAR/Kafka/登记恢复、非空迁移 | 跨三服务补建；先证明历史回执和本地原子性，再扩实际进程；同类失败两次复核 | in_progress |
| WATERMARK：可信对账完整性 | 调用者字符串不能关窗；来源关闭/库存过账/来源回执可核对；未追平不导出完成 | 来源命令/Inbox/Outbox/快照和对账；不引入第二库存权威 | 真实数据库缺失/迟到/重复及重启 | 先确认权威数据和截止点；重大兼容变化再做差异评审 | pending |
| TRANSFER：公开序列调拨闭环 | 原转移/SN/epoch封闭及目的确认；重复不双重授权；数量身份一致 | 既有serial transfer、源释放、公开单据与恢复 | MySQL、真实登记HTTP、公开入口 | 跨服务接线，不扩未批准退货业务 | pending |
...
OUT先补历史发运凭证、持久恢复器、查询和审计接线，然后定向测试。PICK未变证据复用，SHIP修改的共同路径补测。当前整体保持in-progress。
