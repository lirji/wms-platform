...
| 4 | R13–R15 | 消息闭环、TC/序列号服务、恢复与清理任务 | in progress；main 3e2c720 已发布普通消息/多 Cell 路由、真实 TM/TC/RM、出库授权、序列号分批收货/质检/上架/源释放、完整身份盘点及逐身份恢复/占用保护。仍待序列号 PICK/SHIP、可信水位、公开序列调拨、TC 终态通知/资源迁移与取消补偿；详见 [当前状态](DELIVERY_STATUS.md) |
| 4 | R21/R24 | 就绪/观测、正式装配测试、容量执行器 | 核心已提交并本地验证；消息积压/采样有效性指标和告警检查已定向通过，生产通知未接；容量目标未签署，执行器测试不等于WMS容量达标 |
| 5 | R22–R23 | UTC 序列化、操作者审计 | R23 已验证；R22 数据库来源/UTC映射/历史固定偏移/跨JVM分页及 main 3e2c720 CI 已通过，剩余整改完成后再结项复验；没有检查或转换生产历史时区 |
...
阶段提交 f9710ef 已进入远程 main。用户确认质检按每次收货分批；R13 先以原收货 commandId 绑定不可变库存维度，结果版本表示该批累计 accepted/rejected（总数不能超过原批数量，未检部分保持 HOLD）。上架必须绑定该批并不超过该批已生效合格量；新版结论不得将合格量降到已经确认上架的数量以下。旧无库存上下文命令不能猜测补桶，旧行级路径仅保留消息关闭时的兼容行为。之后连续补 R14/R15/R22 与组合验收，无需再等确认。

## R14/R15 盘点逐身份持久恢复

[恢复规则与证据](../../implementation/COUNT_SERIAL_RECOVERY.md)新增V040行意图与逐SN意图，固定首次调整操作/主体/观察/快照，网络在事务外逐身份执行；DONE结果齐备后数量、实际归属代际、身份、原流水与APPLIED同事务提交。正常登记等待不耗行失败预算，原实物未授权先等待；租约接管后的旧回执不覆盖，人工重排沿用企业仓权限与原审计事务。原MISSING命令可返回历史盘亏确认，后续盘盈不丢原恢复凭证，也不回退当前全球归属。

05:56:11真实数据库/HTTP/登记Jar组合34IT通过，必需108/API88；52表迁移包含新表实际JSON/epoch/次数逐字段复制。05:57:19最后补充行级路由校验10IT通过，准备提交发布。序列PICK/SHIP、公开序列调拨、可信水位、TC资源/Fence迁移、晚取消补偿和最终组合验证仍继续，四项未整体关闭。

## 剩余整改有限验收（2026-09-13 恢复）

用户授权按修改后的工作流继续 R13/R14/R15/R22。仅补齐已批准运行链路，不新增中间件或全仓重构。测试使用隔离真实组件，正常 Git 发布；生产/共享数据操作、未定保留期限、OQ-03、硬件和容量签署保留原边界。

| 切片 / 结果 | 有限验收 | 必要依赖与责任路径 | 验证 / 环境 | 投入与复核点 | 状态 |
| --- | --- | --- | --- | --- | --- |
...
| TRANSFER：公开序列调拨闭环 | 原转移/SN/epoch封闭及目的确认；重复不双重授权；数量身份一致 | [公开序列调拨](../../implementation/SERIAL_PUBLIC_TRANSFER.md)、源释放、公开单据与恢复 | MySQL、真实登记HTTP、公开入口 | 跨服务接线，不扩未批准退货业务 | complete，main7659d34；CI34731081335成功 |
| TC：原资源与Fence迁移 | 可靠终态通知；原XID/branch/资源身份保持；未终结拒绝切流；恢复不丢回调 | RM意图、TC审计、仓路由及迁移门禁 | 隔离真实TC/RM/双库及重启 | 先验证回调寻址与终态证据，不能仅复制表关闭验收 | complete：a3b4c65；CI34732426303成功 |
| COMP：全局提交后取消恢复 | 不向已提交TCC伪发Cancel；已知未执行额度补偿一次；实物未知保留处理中；持久重试审计 | 取消/执行授权/库存预占及原执行事实 | 真实数据库并发、部分执行/未知结果与重放 | 需要新业务取舍时只请求该差异，独立授权工作继续 | complete：f043117；CI34734659069成功 |
| FINAL：组合与交付 | 契约/迁移/文档/必需IT齐备；最终组合、main发布及CI；R22结项范围明确 | 上述切片通过，复用未变证据 | 默认verify一次、独立profile、smoke、必要前端和CI | 不重复采样或在构建中编辑；未签署生产目标不声称通过 | complete：有限范围已发布。2026-09-13 用户授权不等34736085774。不是50 AC |

OUT已由dd22cd0/1dd3d18实现并经365a1eb发布main；WATERMARK经2e7451c/051a7eb发布main，真实进程及非空迁移证据见对应专题。051a7eb的main CI34729813580全部成功。TRANSFER7659d34已发布main，CI34731081335成功；TC a3b4c65、COMP f043117 均已发布且CI成功。控制台公开契约 a619d3f 已推 main。有限范围 R13/R14/R15/R22 代码已发布；50 AC / 现场 / 容量仍 open。回执见 [FINAL_RECEIPT](FINAL_RECEIPT.md)。

## 序列盘亏与占用前置校验

盘点实见集合即使数量不变，MISSING加FOUND仍会撤销原身份授权。当前预占只绑定数量桶，不能猜测缺失的是自由身份；有预占或设备自由领取且存在MISSING时，在创建逐身份意图前拒绝，先释放/重分配。没有远程登记、原身份和数量保持不变；占用解除后允许固定原调整。

2026-09-13 06:00:56，`/tmp/wms-count-reserved-it.log` BUILD SUCCESS（CountSerialIT7、CountIT3、真实登记进程1），必需109。测试涵盖两类占用、净数量不变的身份替换、无恢复意图副作用和解除后可继续；占用状态为隔离数据库夹具，未称新的真实TCC验收。


## 序列PICK阶段完成（2026-09-13 06:23）

[序列出库设计与证据](../../implementation/SERIAL_OUTBOUND_DESIGN.md)：公开查询返回当前桶可选SN和实际epoch，PICK来源T1固定原任务/订单行/选择，库存T2数量/预占/具体SN同事务，来源T3逐身份获得已拣额度。内部序列出库V2防旧消费者静默忽略身份，普通V1摘要不变。

来源/库存最终写失败回滚、跨任务/订单行及错epoch、原重放已通过；实际outbound/inventory Jar+Kafka两库的重启恢复与公开查询通过。带数据53表迁移通过。06:22:49补充V2异常消息隔离及HTTP数量不匹配400通过，见 `/tmp/wms-serial-pick-probes-it.log`。112必需用例清单、89路径；SHIP和可信水位仍未接通，不宣称完整出库或R13/R14/R15完成。
