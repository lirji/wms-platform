...
| 4 | R13–R15 | 消息闭环、TC/序列号服务、恢复与清理任务 | in progress；普通RECEIVE/分批QUALITY/PUTAWAY及PICK/SHIP/CANCEL真实双进程通过；七个handler已有有界执行器（归档仅候选）；完整授权、原生RM、持久TM执行及原XID/branch到出库授权真实进程验证通过；多cell普通库存路由已发布；按收货批次的序列号观察与身份质检已定向验证并提交；序列号上架已通过真实进程与事务故障验证；源释放持久事实/历史凭证重放/隔离恢复已验证。仍待序列出库、公开序列调拨接线、TC终态通知/资源迁移、取消补偿、盘点逐身份登记与可信水位 |
