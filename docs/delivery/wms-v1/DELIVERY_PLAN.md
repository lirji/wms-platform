...
| 4 | R13–R15 | 消息闭环、TC/序列号服务、恢复与清理任务 | in progress；R13 RECEIVE 双进程真实MQ闭环通过，质量/出库待补；R15四个handler已通过，盘点恢复含预算/代际/Outbox；其余待接线；R14登记服务HTTP/独立数据库/鉴权已定向通过，库存适配与真实TM/TC待接通 |
