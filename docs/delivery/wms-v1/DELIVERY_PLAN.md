...
| WATERMARK：可信对账完整性 | 调用者字符串不能关窗；来源关闭/库存过账/来源回执可核对；未追平不导出完成 | 来源命令/Inbox/Outbox/快照和对账；不引入第二库存权威 | 真实数据库缺失/迟到/重复及重启 | 先确认权威数据和截止点；重大兼容变化再做差异评审 | local_verified |
...
OUT已由dd22cd0/1dd3d18实现并经365a1eb发布main，真实进程和非空迁移证据见SERIAL_OUTBOUND_DESIGN。当前进入WATERMARK，整体保持in-progress；main CI34725376702已全部成功。
