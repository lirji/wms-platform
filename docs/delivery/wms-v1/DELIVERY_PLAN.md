...
| 4 | R13–R15 | 消息闭环、TC/序列号服务、恢复与清理任务 | in progress；RECEIVE/分批QUALITY/PUTAWAY双进程通过；登记HTTP/有界调用/持久化恢复与受审计重排已验证；R15七个handler已有执行器（归档仅候选）；TC只读审计/有界恢复和分支屏障已真实TC/数据库定向验证；仍待出库/序列号观察、盘点逐身份登记、可信水位与真实TM/RM/授权传播 |
