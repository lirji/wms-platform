...
| FINAL：组合与交付 | 契约/迁移/文档/必需IT齐备；最终组合、main发布及CI；R22结项范围明确 | 上述切片通过，复用未变证据 | 默认verify一次、独立profile、smoke、必要前端和CI | 不重复采样或在构建中编辑；未签署生产目标不声称通过 | complete：有限范围已发布；控制台a619d3f的CI34736085774运行中。不是50 AC |
