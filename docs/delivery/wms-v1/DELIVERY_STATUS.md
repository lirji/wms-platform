...
- 状态：in-progress；阶段：S8/S9 已合入远程 main；route-gate 热修复 + AC-24 HTTP IT 在 `feat/wms-s9-02-route-gate`。
- 用户工作区 `main` 未切换（含未跟踪 `docs/design/11-edge-resilience.md`）。
- 基线 `origin/main`=`2d270ba`。本分支含 `8351fd0` route-gate 与 AC-24 HTTP 导出。
...
| 门禁 | 状态 | 证据/下一步 |
| --- | --- | --- |
| EG-01 工程/CI | running | `2d270ba` main verify `34671971183` 仍 in_progress；`feat/wms-s9-02` tc-it 已因 route-gate bug 失败 |
| EG-05 外部与非功能 | running | S9-01 agreed-peak 仍 blocked；S8-05 仍 blocked |
| Git发布 | pending | 热修复与 AC-24 未进远程 main |
...
- AC-24：ISO cutoff 可解析；测试 JWT 走 POST/GET，不把数量写成金额；跨仓 403。不是 WMS+recon 进程联调。
- 本地：`WarehouseRouteGateTest` 1/0；`TccFenceShardingIT` 1/0（16.90s）；`WarehouseMigrationIT` 2/0。

## 未完成

- 等 `34671971183` 结束后把本分支合入 main。S8-05。S9-01 签署峰值。S9-05 50 AC 全量证据。OQ-03。UI accepted。
