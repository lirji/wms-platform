...
- R13：收货双进程闭环、投影、审计重试已通过；质检/上架/出库等仍待。质检按收货分批还是整行的必要业务口径待答。
- R14：登记服务真实 HTTP/数据库/鉴权定向通过；库存适配与真实 TM/TC 待接。
- R15：四个任务已接通；序列号恢复、内部对账、归档计划仍待。
- R22：UTC 与历史时间兼容未实施。仓迁移允许表清单遗漏另需修复。
- Git：fix/backend-review-remediation 基于 db02821，13 个既有任务提交；正在提交登记服务批次并执行阶段发布回归。main 尚未包含本轮提交，远程 CI 尚未执行。
