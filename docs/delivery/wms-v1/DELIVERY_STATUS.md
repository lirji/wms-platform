...
远程main已发布COMP `f043117`（CI [34734659069](https://github.com/lirji/wms-platform/actions/runs/34734659069)成功）和控制台公开契约 `a619d3f`。控制台 verify [34736085774](https://github.com/lirji/wms-platform/actions/runs/34736085774)运行中。PDA 拣/发与 FINAL 回执在 feat/console-public-serial-ops，等该 CI 结束后再合 main。不是50 AC。
...
| 序列号盘点 | 完整集合/空集合观察、逐身份持久恢复、原 epoch/原操作重放与占用保护 | [观察输入](../../implementation/COUNT_SERIAL_OBSERVATION.md)、[逐身份恢复](../../implementation/COUNT_SERIAL_RECOVERY.md) |
| 数据时间与资源 | UTC 映射、历史固定偏移策略、连接池/缓存与有界恢复等已有验证 | [数据库时间](../../implementation/DATABASE_TIME.md)、[整改记录](BACKEND_REMEDIATION.md) |

以上为已实现且有阶段证据的能力范围。默认配置不自动启用消息、原生 RM 或执行器；真实数据库/JAR 测试与现场用户流程验收仍须区分。
...
单桶完整 SN 观察最多 200 个；更大集合的分段协议未实现，不能拆成多次“完整集合”提交规避限制。仓迁移仍受在途原生 RM、TC 终态/Fence 和共享目录准备门禁约束。
...
以下保留原提交、测试范围和当时结论，供追溯使用。旧“未完成”、运行中的 CI 与授权措辞仅适用于当时阶段；当前状态以上面三节为准，不要求恢复已完成步骤。
