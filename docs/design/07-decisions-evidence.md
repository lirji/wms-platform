...
| ID | 待确认 | 本版提案 | 阻塞范围 |
| --- | --- | --- | --- |
| OQ-01 | 企业内序列号唯一范围 | enterprise+SKU+serial | 序列号表迁移/收货功能 |
| OQ-02（已关闭） | 跨仓事务选型与成功屏障 | 已批准Seata TCC；TC成功且全部仓确认后执行，其余本地事务+可靠消息 | 不再阻塞选型；具体版本/适配仍需S0 |
| OQ-03 | 自营货权主体数、商品单位及效期截止规则 | 保留 owner；精确基础单位；日期转 UTC 留规则版本 | 业务种子/生产接入 |
| OQ-04 | 现有 OMS 是否拥有履约协调 | 新项目暂持有，可通过契约交还 OMS | 外部全局权威确定 |
| OQ-05 | 仓数、订单/扫描峰值、热点分布、SLO、RTO/RPO | 文档合成负载和候选目标 | 容量签署/生产拓扑 |
| OQ-06 | 冻结盘点及迁移可接受停写窗口 | 局部冻结、维护窗口迁移 | 现场验收/数据迁移 |
| OQ-07 | 对账系统 quantity 扩展和数据提供方 | 接 recon-platform，金额路径不变 | 真实跨系统对账验收 |
| OQ-08 | 代码仓库、远程及 CI 平台 | 当前GitHub origin为空远程、本地任务分支，CI采用GitHub Actions | 实施分支/CI发布 |
| OQ-09 | WCS/设备协议及隔离环境 | 离线只记录意图；未知结果人工核实 | 真实设备验收 |
| OQ-10 | 保留期、审批分权、组织身份提供方 | 最小权限，接已有认证优先；期限不编造 | 生产安全/生命周期 |

这些项不影响文档完整交付；实施时仅暂停依赖该决定的切片，继续其他已授权工作，不把提案视为已确认业务事实。
