...
| OQ-10 | 部分关闭 | 认证接入采用 OIDC 资源服务器；具体 issuer/client/仓范围权限在 S1-02 用环境配置接入已有提供方，失败不得回退免认证。未指定 IdP 产品名。保留期与审批分权仍待确认 | S1 权限集成；生产生命周期仍待 |

未关闭项实施时只暂停依赖该决定的切片。已关闭项按上表落库与实现，不再当作提案。

## 3.1 本轮业务决定批准

用户确认：“TM 归属：wms-fulfillment；认证：OIDC 提供方；序列号唯一范围：企业+SKU+serial”。据此关闭 OQ-01/OQ-04，并关闭 OQ-10 的身份提供方式；不把未点名的 IdP 产品或 OQ-03 单位/效期规则写成已批准。
