...
class WarehouseMigrationIT {
    ...
    void twoPhysicalDatabasesSwitchEpochAndRejectOldWrites() {
        ...
        sourceJdbc.update("UPDATE serial_release_intent SET target_warehouse_id='WH-B' WHERE id='RELEASE-MIGRATION'");
        sourceJdbc.update("INSERT INTO serial_transfer_command(id,enterprise_id,warehouse_id,context_hash,payload,state,next_check_at,version,created_at,updated_at) VALUES('TRANSFER-CMD-M','ENT-1','WH-A',?,CAST(? AS JSON),'PENDING',UTC_TIMESTAMP(6),7,UTC_TIMESTAMP(6),UTC_TIMESTAMP(6))", "d".repeat(64), observation);
        ...
        for(String table:java.util.List.of("count_adjustment_intent","count_serial_intent","serial_pick_fact","serial_shipment_intent","reconciliation_history_guard","reconciliation_cutoff","reconciliation_collection_audit","serial_transfer_command"))
