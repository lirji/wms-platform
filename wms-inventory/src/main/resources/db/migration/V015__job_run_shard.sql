-- S7-02 业务任务身份与分片。XXL 触发不等于分片完成。活跃分片唯一。
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='业务任务运行，调度成功不等于本行完成';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='业务分片租约与检查点，回收后旧 fence 失效';
