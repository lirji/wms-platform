...
public final class SeataRmDriver implements AutoCloseable,com.lrj.wms.runtime.observability.RuntimeDependencyCheck {
    ...
    public SeataRmDriver(RuntimeTccCoordinator coordinator,String group,List<String> servers,List<String> xidAddresses) {
        ...
        synchronized(SeataRmDriver.class) {
            ...
            byte[] identity=java.util.HexFormat.of().parseHex(com.lrj.wms.inventory.inventory.domain.CommandDigest.v1Parts(coordinator.actionName()));
            String applicationId="wms-rm-"+java.util.Base64.getUrlEncoder().withoutPadding().encodeToString(java.util.Arrays.copyOf(identity,16));
            ...
            try {
                ...
                resource.setActionName(coordinator.actionName());resource.setResourceGroupId(group);resource.setAppName(applicationId);
            ...
            } catch(ReflectiveOperationException failure) {client.destroy();throw new IllegalStateException("RM回调声明不完整",failure);}
        }
        probe.scheduleWithFixedDelay(()->{
            try {
                ...
                healthyAt=status==org.apache.seata.core.model.GlobalStatus.Finished?System.nanoTime():0;
    ...
    @Override public synchronized void close(){if(!closed){closed=true;healthyAt=0;probe.shutdownNow();client.destroy();}}
}
